...
def gps_json_to_pd(json_data, total_iterations):
    np_array = np.zeros((total_iterations + 1, 11), dtype=np.float64)
    ...
    for iteration_no_str, gps_data in json_data.items():
        ...
        np_array[iteration_no][10] = gps_data['Ignore']
...
def ignore_field(column_names):
    for column_name in column_names:
        if 'ignore' in column_name.lower():
            return column_name
    return None
...
def inject_faults(all_data_pd,
                  ...
                  gaussian_y,
                  gaussian_z):
    '''returns accel data where fault_node is failed'''
    new_data = all_data_pd.copy()
    ignore_column = ignore_field(fault_node)
    if ignore_column is not None:
        new_data.loc[start_time:end_time, ignore_column] = 1
...
if __name__ == '__main__':
    ...
    mode_data = read_file(MODE_FILENAME)
    ...
    seen_injection_times = set(mode_data)
    next_injection_times = copy(mode_data)
    while len(next_injection_times) != 0:
        time = next_injection_times[0]
        next_injection_times = next_injection_times[1:]

        if not (time + 1) in seen_injection_times:
            next_injection_times.append(time + 1)
            seen_injection_times.add(time + 1)
        if (time - 1) > 0 and not (time-1) in seen_injection_times:
            next_injection_times.append(time - 1)
            seen_injection_times.add(time - 1)

        for node_to_fail in fault_nodes:
            scenarios_considered += 1
            new_accel_data = inject_faults(
                all_inputs,
                time,
                time + INJECTION_TICKS,
                node_to_fail,
                accel_x_node,
                accel_y_node,
                accel_z_node,
            ...
            position = pos_pd.iloc[time].to_numpy().copy()
            velocity = velocity_pd.iloc[time].to_numpy().copy()
            for t in range(0, new_accel_data.shape[0]):
                position += new_accel_data[t]
            print(f'failure @ t = {time}')
            print(f'starting position: {pos_pd.iloc[time + INJECTION_TICKS].to_numpy()} end: {position}')
